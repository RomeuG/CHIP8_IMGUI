#ifndef __IMGUI_FILEBROWSER_HPP_
#define __IMGUI_FILEBROWSER_HPP_
...
inline ImGui::FileBrowser& ImGui::FileBrowser::
operator=(const FileBrowser& copyFrom)
{
        ...
        reserveHeight += GetItemsLineHeightWithSpacing();
    {
        ...
        for (auto& rsc : fileRecords_) {
            ...
            if (IsItemClicked(0) && IsMouseDoubleClicked(0) && rsc.isDir) {
                setNewPwd = true;
                newPwd =
                    (rsc.name != "..") ? (pwd_ / rsc.name) : pwd_.parent_path();
            }
        }
    }

...
struct MemoryEditor {
    ...
    void (*WriteFn)(u8* data, size_t off,
                    u8 d); // = NULL   // optional handler to write bytes.
    bool (*HighlightFn)(
        const u8* data,
        size_t
            off); //NULL   // optional handler to return Highlight property (to support non-contiguous highlighting).
    ...
    void CalcSizes(Sizes& s, size_t mem_size, size_t base_display_addr)
    {
        ...
        if (OptShowAscii) {
            ...
            if (OptMidColsCount > 0)
                s.PosAsciiStart +=
                    ((Cols + OptMidColsCount - 1) / OptMidColsCount) *
                    s.SpacingBetweenMidCols;
                      ...
                      size_t base_display_addr = 0x0000)
    {
        ...
        if (DataEditingAddr != (size_t)-1) {
            ...
            if (ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_UpArrow)) &&
                ...
                DataEditingTakeFocus = true;
                           ...
                           ImGui::GetKeyIndex(ImGuiKey_DownArrow)) &&
                       DataEditingAddr < mem_size - Cols) {
                data_editing_addr_next = DataEditingAddr + Cols;
                DataEditingTakeFocus = true;
            } else if (ImGui::IsKeyPressed(
                           ImGui::GetKeyIndex(ImGuiKey_LeftArrow)) &&
                       DataEditingAddr > 0) {
                data_editing_addr_next = DataEditingAddr - 1;
                DataEditingTakeFocus = true;
            } else if (ImGui::IsKeyPressed(
                           ImGui::GetKeyIndex(ImGuiKey_RightArrow)) &&
        ...
        const ImU32 color_disabled =
            OptGreyOutZeroes ? ImGui::GetColorU32(ImGuiCol_TextDisabled) :
                               color_text;
             ...
             line_i++) // display only visible lines
        {
            ...
            for (int n = 0; n < Cols && addr < mem_size; n++, addr++) {
                ...
                if (OptMidColsCount > 0)
                    byte_pos_x +=
                        (n / OptMidColsCount) * s.SpacingBetweenMidCols;
                ...
                if (is_highlight_from_user_range ||
                    is_highlight_from_user_func || is_highlight_from_preview) {
                    ...
                    struct UserData {
                        ...
                        static int Callback(ImGuiInputTextCallbackData* data)
                        {
                            ...
                            if (data->SelectionStart == 0 &&
                                ...
                                data->InsertChars(
                                    0, user_data->CurrentBufOverwrite);
            ...
            if (OptShowAscii) {
                ...
                if (ImGui::InvisibleButton(
                        "ascii", ImVec2(s.PosAsciiEnd - s.PosAsciiStart,
                                        s.LineHeight))) {
                    DataEditingAddr = DataPreviewAddr =
                        addr + (size_t)((ImGui::GetIO().MousePos.x - pos.x) /
                                        s.GlyphWidth);
                    DataEditingTakeFocus = true;
                }
                ImGui::PopID();
                for (int n = 0; n < Cols && addr < mem_size; n++, addr++) {
                    ...
                    draw_list->AddText(
                        pos, (display_c == '.') ? color_disabled : color_text,
        ...
        if (OptShowDataPreview) {
            ...
            if (ImGui::BeginCombo("##combo_type",
                                  DataTypeGetDesc(PreviewDataType),
            ...
            ImGui::PopItemWidth();
            ImGui::SameLine();
            ImGui::PushItemWidth((s.GlyphWidth * 6.0f) +
                                 style.FramePadding.x * 2.0f +
                                 style.ItemInnerSpacing.x);
            ...
            if (has_value)
                DisplayPreviewData(DataPreviewAddr, mem_data, mem_size,
                                   PreviewDataType, DataFormat_Dec, buf,
            ...
            if (has_value)
                DisplayPreviewData(DataPreviewAddr, mem_data, mem_size,
                                   PreviewDataType, DataFormat_Hex, buf,
                                   (size_t)IM_ARRAYSIZE(buf));
            ImGui::Text("Hex");
            ImGui::SameLine(x);
            ImGui::TextUnformatted(has_value ? buf : "N/A");
            if (has_value)
                DisplayPreviewData(DataPreviewAddr, mem_data, mem_size,
                                   PreviewDataType, DataFormat_Bin, buf,
                                   (size_t)IM_ARRAYSIZE(buf));
            ImGui::Text("Bin");
            ImGui::SameLine(x);
            ImGui::TextUnformatted(has_value ? buf : "N/A");
    ...
    void* EndianessCopy(void* dst, void* src, size_t size) const
    {
        ...
        if (fp == NULL)
            fp = IsBigEndian() ? EndianessCopyBigEndian :
    ...
    void DisplayPreviewData(size_t addr, const u8* mem_data, size_t mem_size,
                            DataType data_type, DataFormat data_format,
                            char* out_buf, size_t out_buf_size) const
    {
        ...
        switch (data_type) {
            ...
            case DataType_S64: {
                ...
                if (data_format == DataFormat_Hex) {
                    snprintf(out_buf, out_buf_size, "0x%016llx",
                             (long long)int64);
                    return;
                }
                break;
